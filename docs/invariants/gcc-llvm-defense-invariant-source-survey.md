...
| Clang docs: Sanitizer 系列 | user-doc + internals | https://clang.llvm.org/docs/AddressSanitizer.html | ASan/HWASan/MSan/TSan/UBSan/DFSan 各自独立页, 同目录相邻 | sanitizers | shadow 倍率/偏移/poison 颜色; `__asan_*` runtime 入口符号; UBSan trap mode (`-fsanitize-trap=...`) 与 handler 模式区别; 与 `-fno-omit-frame-pointer` / `-fno-optimize-sibling-calls` 的交互 |
...
| `llvm/lib/Transforms/Instrumentation/` | source | https://github.com/llvm/llvm-project/tree/main/llvm/lib/Transforms/Instrumentation | 目录下: `AddressSanitizer.cpp`, `HWAddressSanitizer.cpp`, `MemorySanitizer.cpp`, `ThreadSanitizer.cpp`, `DataFlowSanitizer.cpp`, `SanitizerCoverage.cpp`, `BoundsChecking.cpp`, `KCFI.cpp`, `CFGuard.cpp` — 各 sanitizer / KCFI / BoundsChecking / SanCov 的 IR pass | sanitizers, KCFI, SanCov | 哪些访问被插桩 (例如跳过 `safebuffers` 函数); 哪些指令保留为 trap; KCFI 元数据写入 `kcfi_type` 与 prologue 校验序列 |
| `llvm/lib/Target/AArch64/` | source | https://github.com/llvm/llvm-project/tree/main/llvm/lib/Target/AArch64 | 关键文件: `AArch64FrameLowering.cpp`, `AArch64MachineFunctionInfo.h`, `AArch64BranchTargets.cpp`, `AArch64PointerAuth.cpp` — AArch64 后端 BTI/PAC/SCS lowering | BTI, PAC, SCS, SP | "PAC `paciasp/autiasp` 必须在 LR 仍可信的窗口内匹配"; "SCS push/pop 与 unwind 需要 `.cfi_*` 注册"; "BTI landing pad 在所有 indirect target / setjmp 返回点" |
| `llvm/lib/Target/X86/` | source | https://github.com/llvm/llvm-project/tree/main/llvm/lib/Target/X86 | 关键文件: `X86IndirectBranchTracking.cpp` 等 — x86 CET / SHSTK / IBT lowering | CET | `endbr` 插入位置 (函数入口、`setjmp` 回点、可被间接调用 BB); SHSTK 与 alloca / `__builtin_setjmp` 交互 |
| `llvm/lib/Target/RISCV/` | source | https://github.com/llvm/llvm-project/tree/main/llvm/lib/Target/RISCV | RISC-V 控制流强化 lowering: Zicfilp / Zicfiss / 软 SCS | SCS, CFI(硬件) | Zicfilp landing pad (`lpad`) 插入规则; Zicfiss `sspush/sspop` 与 `ssp` 寄存器规则; SCS 软实现使用 `gp/x3`, 链接器需 `--no-relax-gp` |
...
| `clang/include/clang/Basic/` | source | https://github.com/llvm/llvm-project/tree/main/clang/include/clang/Basic | 关键文件: `Sanitizers.def`, `Attr.td`, `AttrDocs.td` — sanitizer kind 表与属性表 | sanitizers, 全部 | 各 sanitizer flag 的 group 关系; 属性参数与适用 decl 类别 |
| `clang/lib/CodeGen/` | source | https://github.com/llvm/llvm-project/tree/main/clang/lib/CodeGen | 关键文件: `CGStackProtector.cpp`, `CGCall.cpp`, `CGCXXABI.cpp`, `CGPointerAuth.cpp` — 前端 IR 生成: SP 调度、call 插桩、C++ ABI lowering、PAC 限定符下沉 | SP, CFI, PAC | 前端如何把 `__attribute__((no_stack_protector))` 翻成 IR `nossp`; vtable 与 `cfi-vcall` 元数据生成; PAC 限定符下到 IR ptrauth intrinsic |
...
| `compiler-rt/lib/` | runtime | https://github.com/llvm/llvm-project/tree/main/compiler-rt/lib | 子目录: `asan`, `hwasan`, `msan`, `tsan`, `ubsan`, `cfi`, `safestack`, `shadowcallstack`, `sanitizer_common` — 各 sanitizer / cfi / safestack / scs runtime | sanitizers, CFI, SS, SCS | shadow memory 布局; trap handler 接口; cross-DSO CFI shadow 由 loader 协助维护; SCS guard region allocation 与 thread exit 释放 |
| `llvm/test/Transforms/` | test | https://github.com/llvm/llvm-project/tree/main/llvm/test/Transforms | 子目录: `StackProtector`, `SafeStack`, `KCFI`, `CrossDSOCFI`, `BoundsChecking` — IR 级 lit tests | SP, SS, KCFI, CFI, BS-runtime | 哪些 IR 构造触发 protector / SafeStack 拷贝; KCFI hash 在 prologue 的字节序列; cross-DSO CFI slow path |
...
| `compiler-rt/test/` | test | https://github.com/llvm/llvm-project/tree/main/compiler-rt/test | 子目录: `cfi`, `safestack`, `shadowcallstack`, `asan`, `hwasan`, `ubsan` — runtime 行为 lit tests | CFI, SS, SCS, sanitizers | "X 应触发 X-trap" 的正例; "在 Y 配置下不报"的反例; cross-DSO 行为 |
...
| LLVM Discourse RFC 索引 | RFC | https://discourse.llvm.org/c/clang/6 | 含 KCFI / Structure Protection / arity-aware FineIBT 等 RFC 主线程 | KCFI, STRP, CET | KCFI 设计选型; FineIBT arity 编码协议; Structure Protection 阶段化引入策略 |
