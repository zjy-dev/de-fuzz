...
\begin{itemize}
  \item \textbf{An oracle for silent failure (main line).} A sound,
  ...
  (\S\ref{sec:oracle}).
  \item \textbf{Systematized security invariants (support).} A bottom-up
  ...
  \item \textbf{An explicitly-orchestrated agent system (moat).} A deterministic
  ...
  \item \textbf{An agentic loop (vehicle).} Oracle-grounded, coverage-guided
...
orchestrator enumerates the entire checker catalog into a deterministic
...
handed to a minimizer agent, a non-violation goes through a feedback agent back
to the next round. The full trajectory is versioned into a per-run checkpoint
chain, so it is reproducible, ablatable, and auditable.

The system architecture is divided into two main components (Figure~\ref{fig:architecture}): a Go deterministic
...
adapters read a single source of truth for checker metadata, so adjudication
...
description; \emph{analogy} is a semantic-isomorphism gate that decides whether a
retrieved code block instantiates the same failure shape; \emph{specialize}
instantiates the invariant on the concrete target source; and \emph{entailment}
checks that the instantiated statement actually follows
(Figure~\ref{fig:specgen-pipeline}). The analogy gate is essential: roughly
94\% of BM25 hits are lexical collisions, and only the semantic gate keeps
generation quality up.
...
(threshold 85.0) confirms it does not duplicate an existing seed or manual rule.
...
honestly when the mechanism is off or the seed does not exercise it, and is
...
fixed positions and do semantic judgment; the pipeline keeps control. (3) Agents
never call each other directly---they communicate through shared state. (4)
Correctness is held by deterministic evidence: the agent proposes, the oracle
adjudicates.

\subsection{Explicit orchestration and the blackboard}
The linkage between agents is a real closed loop---feedback guides the next
...
guidance. This design provides three critical properties that elevate agent-based bug hunting into a rigorous method: \emph{reproducibility} (pin a blackboard version and any agent's input is
...
Instead of letting the agent pick an ISA, we statically bind each checker to its applicable ISA set, restricting the agent to the semantic task for which it is well-suited: ``which checkers can this seed trigger?'' Choosing a checker
automatically claims all ISAs bound to it, so the agent never touches the raw ISA
...
full ISA fan-out and the cross-ISA silent-failure signal is preserved by
metadata rather than by agent diligence.
...
execution under QEMU user mode. We report the two research questions the
current artifact supports.

\paragraph{RQ1 --- Real bugs} Across GCC, LLVM, \texttt{lld}, and
\texttt{compiler-rt}, DeFuzz produced 29 distinct silent-failure defects that
cleared a five-item admission gate (concrete violated invariant, concrete
impact, a source-level reason no later layer rescues the failure, at least one
verbatim evidence site, and a reproduction recipe); one further candidate was
retracted on closer inspection and is excluded from the count. Each defect is
grounded in a deterministic binary or runtime signal rather than in model
output. The corpus spans 17 defense mechanisms and 15 instruction-set targets,
confirming that silent failures are not confined to a single cell of the
mechanism~$\times$~ISA matrix. Figure~\ref{fig:bug-stats} breaks the corpus down
by producing toolchain, defense mechanism, and affected ISA; GCC accounts for
19 defects and LLVM-family projects for the remaining 10, and \texttt{ibt}
(Intel CET / RISC-V Zicfilp landing pads) is the most frequently violated
mechanism. Seventeen defects are ISA-specific and twelve are generic. The
defects concentrate on x86-64, RISC-V, and AArch64, the three targets with the
densest control-flow-integrity surface. Table~\ref{tab:corpus} summarizes the
composition. Following the project's responsible-disclosure policy, findings
remain private until the holder files upstream; at the time of writing one
defect has been reported upstream and the rest are pending disclosure, so we
report neither CVE identifiers nor upstream-confirmation counts here.
...
\paragraph{RQ2 --- Invariant generation quality} The cross-mechanism pipeline
