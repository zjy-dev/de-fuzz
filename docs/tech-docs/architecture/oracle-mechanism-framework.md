...
oracle.Oracle                                       (内部 facade，引擎只看到这个)
  └─ MechanismOracle                                (per-mechanism 聚合器)
        ...
        └─ []InvariantChecker
              ├─ Category() ∈ {Enablement, Static, Dynamic}
              ├─ ID()      e.g. "INV-SP-L01"
...
### 2.2 Category 是调度阶段，不是分类标签

| Category | 现状 | 何时跑 | 何时短路 |
| --- | --- | --- | --- |
| `CategoryEnablement` | 暂未注册 checker | Phase 1 | 任一 `Fail` 在 `PolarityPositive` 下视为机制未启用 → 返回 `nil bug` |
| `CategoryStatic` | `StackChkSymbolsChecker`、`MainNoCanaryChecker` | Phase 2 | 无短路；任何 `Fail` 进入聚合 |
| `CategoryDynamic` | `DynamicBufferSearchChecker`、`EpilogueCanaryScrubChecker` | Phase 3 | 无短路；通过 `CheckContext.Cache` 复用 binary search 结果 |

调度顺序在 `mechanism.go:82-101`，目前 **顺序串行**——并行作为 follow-up 显式排除（ADR-003 §3.5）。
...
`MechanismOracle.Analyze`（`mechanism.go:97-114`）实施 ADR-003 §3.2 推荐的 **BLOCKING + OR**：

1. Phase 1 Enablement 任一 Fail (positive polarity) → 返回 `nil bug`（机制未启用属配置错误，不是漏洞）；
2. Phase 2 + Phase 3 收集所有 verdict；
3. **任一 `VerdictFail`** → 返回 `*Bug`，描述由 `formatDescription` 渲染；否则返回 `nil`。
...
3. 在对应 `<mechanism>_oracle.go` 的 `mechanism()` 函数 Checkers 列表里追加该 checker；保留按 Category 顺序声明的习惯（Enablement → Static → Dynamic）。
