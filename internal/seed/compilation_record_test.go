...
func TestCompilationRecordPersistence(t *testing.T) {
	...
	record := &CompilationRecord{
		...
		ProfileAxes:       map[string]string{"policy": "strong", "threshold": "8"},
		...
		LLMCFlags:         []string{"-fstack-protector-all"},
		AppliedLLMCFlags:  []string{"-fstack-protector-all"},
		DroppedLLMCFlags:  []string{"-fno-stack-protector"},
		LLMCFlagsApplied:  false,
		EffectiveFlags:    []string{"-B/tmp/gcc", "-Wall", "-O0", "-fstack-protector-strong", "--param=ssp-buffer-size=8"},
		Stdout:            "stdout",
		Stderr:            "stderr",
	}
