...
func TestCFGGuidedEngine_WithCFGAnalyzer(t *testing.T) {
...
`
	...
	// Create CFG analyzer
	analyzer, err := coverage.NewCFGGuidedAnalyzer(cfgPath, []string{"test_func"}, "", mappingPath)
	if err != nil {
	...
	if engine.cfg.CFGAnalyzer == nil {
		t.Error("CFGAnalyzer should be set")
	}

	// Verify we can get function coverage
	funcCov := engine.cfg.CFGAnalyzer.GetFunctionCoverage()
