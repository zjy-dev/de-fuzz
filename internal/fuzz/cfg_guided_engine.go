// Package fuzz provides CFG-guided fuzzing engine.
package fuzz
...
// CFGGuidedConfig holds configuration for CFG-guided fuzzing.
...
func (e *CFGGuidedEngine) Run() error {
	...
	for {
		...
		target := e.cfg.CFGAnalyzer.SelectTarget()
		if target == nil {
...
func (e *CFGGuidedEngine) processInitialSeeds() error {
	...
	for {
		...
		if report != nil {
			...
			e.cfg.CFGAnalyzer.RecordCoverage(int64(s.Meta.ID), coveredLines)
		}
	...
	funcCov := e.cfg.CFGAnalyzer.GetFunctionCoverage()
...
func (e *CFGGuidedEngine) solveConstraint(target *coverage.TargetInfo) (bool, error) {
	...
	ctx, err := prompt.BuildTargetContextFromCFG(target, baseSeed, e.cfg.CFGAnalyzer)
	if err != nil {
	...
	for retry := 0; retry < e.cfg.MaxRetries; retry++ {
		...
		// TODO: Implement divergence analysis using uftrace
		// For now, just retry with refined prompt
...
func (e *CFGGuidedEngine) tryMutatedSeed(s *seed.Seed, target *coverage.TargetInfo) (bool, bool, error) {
	...
	// Record new coverage
	oldCount := len(e.cfg.CFGAnalyzer.GetCoveredLines())
	e.cfg.CFGAnalyzer.RecordCoverage(int64(s.Meta.ID), coveredLines)
	newCount := len(e.cfg.CFGAnalyzer.GetCoveredLines()) - oldCount
...
func (e *CFGGuidedEngine) saveState() {
	...
	if e.cfg.MappingPath != "" {
		if err := e.cfg.CFGAnalyzer.SaveMapping(e.cfg.MappingPath); err != nil {
